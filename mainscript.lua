...
local normalSize, minimizedSize = UDim2.new(0, 420, 0, 550), UDim2.new(0, 150, 0, 40)
...
local sea3Page = createPage("Sea 3")
...
local function addBtn(txt, color, parent)
    ...
    btn.MouseEnter:Connect(function()
        btn:TweenSize(UDim2.new(0.98, 0, 0, 42), "Out", "Quad", 0.2, true)
    end)
    
    btn.MouseLeave:Connect(function()
        btn:TweenSize(UDim2.new(0.95, 0, 0, 40), "Out", "Quad", 0.2, true)
    end)
...
local jBtn = addBtn("⬆️ Infinite Jump", Color3.fromRGB(100, 200, 255), movePage)
local iJ = false
jBtn.MouseButton1Click:Connect(function() iJ = not iJ end)
...
local nBtn = addBtn("🔥 No Clip", Color3.fromRGB(200, 100, 255), movePage)
local ncl = false
nBtn.MouseButton1Click:Connect(function() ncl = not ncl end)
